...
void LinkedList::Append(Bid bid) {
	...
	//attempt to catch if there is a duplicate bid

	/*for(int i = 0; i < LinkedList.size(); ++i) { //check if Bid already exists in system
		if (curNode->bid.bidId == bid){
			cout << "There is a matching bid already in this list. Please pick another to append." << endl;
		}
		else {
...
*/
    ...
    //else 
		...
		if (tail != nullptr) { //ensure tail exists
		tail->next = newNode;
		tail = newNode;
		}

		else {
			cout << "Tail does not exist" << endl;
		}
	}
...
*/
	...
	else { //list is empty and head = null
		head = newNode;
		tail = newNode;
	}
...
void LinkedList::PrintList() {
	...
	if (curNode == nullptr) { //ensure list is not empty
		cout << "List is empty. Please add items" << endl;
	}

	else {
		while (curNode != nullptr) {
			cout << "BidID:" << curNode->bid.bidId;
...
void LinkedList::Remove(string bidId) {
    ...
    // start at the head
	Node* curNode = head;
	Node* sucNode; //Create successor node
			...
			while(curNode != nullptr) {
				if (curNode == nullptr) {
					...
					if (curNode->next->bid.bidId == bidId) {
						Node* temp = curNode->next; //temp node to store next
						curNode->next = temp->next; //make current node point beyond next node
						temp = nullptr; //empty temp
						--size;
						cout << bidId << " removed successfully." << endl;
						return;
...
Bid LinkedList::Search(string bidId) {
	...
	Node* curNode = head;
	if (curNode == nullptr) {
		cout << "List is empty. Please populate" << endl;
		}

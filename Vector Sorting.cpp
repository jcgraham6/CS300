...
int partition(vector<Bid>& bids, int begin, int end) {
	...
	while (!done){
       ...
       // else swap the low and high bids (built in vector method)
			...
			swap(bids[low].title,bids[high].title);
...
void selectionSort(vector<Bid>& bids) {
	...
	for (unsigned pos = 0; pos < size_t-1; ++pos) {
		...
		//swap current minimum with smaller one found
		 swap(bids[pos].title,bids[min].title); //why does swap return an error?
...
int main(int argc, char* argv[]) {
    ...
    case 2:
        ...
        break;
    default:
        csvPath = "eBid_Monthly_Sales.csv"; //eBid_Monthly_Sales_Dec_2016.csv
    }
    ...
    while (choice != 9) {
        ...
        case 3:
        	...
        	cout << "Number of Bids Read - " << bids.size() << endl;
